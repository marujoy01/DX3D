...
public:
	...
	HRESULT	Bind_Vector(const _char* pConstantName, const _vector* pVector);
	...
	//					셰이더파일에 있는 변수의 이름, 타입, 사이즈)
	HRESULT Bind_RawValue(const _char * pConstantName, const void* pData, _uint iSize);	// 타입들의 낮은 수준의 임의의 사이즈를 셰이더로 다 던질 수 있다.
