...
HRESULT CVIBuffer_Cell::Initialize_Prototype(const _float3* pPoints)
{
	...
	m_iNumVertices = 3;	// 정점 3개
	m_iStride = sizeof(VTXPOS);

	m_iNumIndices = 4; // 인덱스 4개(라인스트림) : 0, 1, 2, 0 인덱스 순서대로 삼각형 모양 선을 긋는다고 생각하면 됨
	m_iIndexStride = 2; 
	m_eIndexFormat = m_iIndexStride == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
	m_eTopology = D3D_PRIMITIVE_TOPOLOGY_LINESTRIP; // 라인스트림 방식으로 삼각형을 그리겠다.

#pragma region VERTEX_BUFFER
	ZeroMemory(&m_BufferDesc, sizeof m_BufferDesc); // 버퍼 구조체를 채워주자.

	m_BufferDesc.ByteWidth = m_iStride * m_iNumVertices; // 가로 한줄 크기
	...
	VTXPOS* pVertices = new VTXPOS[m_iNumVertices]; // 위치 벡터를 넣어주자.
